...
* **🌐 Двуязычный интерфейс:** Поддержка русского и английского языков с динамическим переключением текста виджета и пунктов меню.
* **💾 Сохранение настроек:** Программа автоматически запоминает позицию на экране, выбранный язык, статус блокировки и опцию кликов мыши (при этом счетчик обнуляется при каждом старте).
* **📥 Работа в системном трее:** Мгновенное сворачивание в трей по клику колесиком мыши (СКМ) или через ПКМ-меню. Подсчет продолжается в фоновом режиме.
* **📝 Двуязычное логирование:** Автоматическое ведение текстовой статистики (`stats_log.txt`) на русском и английском языках с фиксацией времени запуска, выключения и количества кликов. Файл доступен прямо из меню.
...
* **🔒 Удобное управление:** Возможность заблокировать перемещение виджета, приостановить подсчет или включить учет мыши через контекстное меню.
...
| **Время (Time)** | Время запуска и общую длительность сессии | Формат отображения адаптируется под выбранный язык. |
...
Программа подсчитывает **только одиночные физические нажатия клавиш** (зажатие клавиши и автоповтор операционной системы полностью игнорируются во всех режимах).
...
Опция **Учитывать клики мышью** (включается через контекстное меню):
* Дополнительно подсчитывает клики кнопок мыши с сопровождением эффекта круговой ряби внутри куба.
...
| **Клик СКМ (колесико)** | Мгновенно свернуть куб в системный трей |
| **Клик ПКМ** | Открытие контекстного меню настроек |
| **Клик по иконке в трее** | Свернуть куб в трей / Развернуть на экран |

### Меню настроек (ПКМ):
* **Учитывать клики мышью** — включение/выключение подсчета нажатий кнопок мыши.
...
* **Скрыть в трей** — сворачивание куба в системную область уведомлений.
* **Открыть лог статистики** — открытие файла `stats_log.txt` в Блокноте.
* **Язык \ Language** — выбор языка интерфейса (Русский / English).
* **Выход** — сохранение настроек, логирование и закрытие приложения.
